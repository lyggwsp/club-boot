...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel("系统创建菜单[SysMenuCreateReq]")
public class SysMenuCreateReq {


    @ApiModelProperty("菜单名称：通常用于标识菜单项的唯一性或作为菜单项的识别符。")
    @NotBlank(groups = ValidGroup.Insert.class, message = "菜单名称不能为空")
    @Size(groups = ValidGroup.Insert.class, max = 50, message = "菜单名称长度不能超过50个字符")
    private String name;

    @ApiModelProperty(value = "类型,参考MenuTypeEnum类型", example = "1", required = true)
    @NotNull(groups = ValidGroup.Insert.class, message = "菜单类型不能为空")
    private Integer menuType;

    @ApiModelProperty("菜单页面标题：用于显示菜单项的文本或标题。")
    @NotBlank(groups = ValidGroup.Insert.class, message = "菜单标题不能为空")
